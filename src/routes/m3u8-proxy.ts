...
const M3U8_RESPONSE_HEADERS: HeadersInit = {
...
async function proxyM3U8(
  event: ReturnType<typeof defineEventHandler> extends (e: infer E) => any
    ? E
    : never,
): Promise<Response> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw createError({
        ...
        statusMessage: `Failed to fetch M3U8: ${response.statusText}`,
      });
    }

    const m3u8Content = await response.text();

    // Derive the proxy base URL from request headers (web-standard: no Node.js APIs)
        ...
        if (!isCacheDisabled()) {
          ...
          Promise.all(
            segmentUrls.map((segmentUrl) =>
              prefetchSegment(segmentUrl, headers),
            ),
          ).catch((error) => {
